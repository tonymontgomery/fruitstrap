...
bool found_device = false, debug = false, verbose = false, quiet = false, unbuffered = false;
char *app_path = NULL;
char *device_id = NULL;
char *args = NULL;
char *gdb_args = "";
...
mach_error_t operation_callback(CFDictionaryRef dict, int arg) {
    int percent;
    CFStringRef status = CFDictionaryGetValue(dict, CFSTR("Status"));
    CFNumberGetValue(CFDictionaryGetValue(dict, CFSTR("PercentComplete")), kCFNumberSInt32Type, &percent);
...
int main(int argc, char *argv[]) {
    ...
    char ch;
    while ((ch = getopt_long(argc, argv, "qvdu:t:b:a:i:g:", global_longopts, NULL)) != -1)
    {
        switch (ch) {
        case 'q':
            quiet = 1;
